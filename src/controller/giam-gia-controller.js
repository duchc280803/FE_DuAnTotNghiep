...
$scope.chonSanPham = function (productId) {
  ...
  $scope.selectAllProducts = $scope.sanPhamDaChon.length === $scope.listProduct.length;
};
...
function checkTenGiamGiaExists(tenGiamGia) {
  return $http.get('http://localhost:8080/api/v1/giam-gia/checkTenGiamGia', {
      params: {
...
function checkGiamGiaSanPhamExists(idsanpham) {
  ...
  return $http.get('http://localhost:8080/api/v1/giam-gia/check-product-record-count', {
      params: {
        idsanpham: idsanpham
      }
    })
...
$scope.themKhuyenMai = function () {
  if (!$scope.maGiamGia || !$scope.tenGiamGia || !$scope.mucGiam || !$scope.hinhThucGiam || !$scope.ngayBatDau || !$scope.ngayKetThuc) {
  ...
  checkTenGiamGiaExists($scope.tenGiamGia)
    .then(function (exists) {
      if (exists) {
        ...
        return checkGiamGiaSanPhamExists($scope.sanPhamDaChon.length > 0 ? $scope.sanPhamDaChon : null);
      }
    })
...
if ($scope.DanhMuc != null) {
  ...
  idDanhMuc = null;
        ...
        $http.post('http://localhost:8080/api/v1/giam-gia/create', dataToSend)
          .then(function (response) {
            ...
            if (confirm('Thêm khuyến mãi thành công.Bạn có muốn chuyển hướng trang')) {
